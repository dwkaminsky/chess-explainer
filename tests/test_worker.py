...
class FakeTasks:
    ...
    def complete_task(self, session, task_id, lease_token, result, **kwargs):
        self.completed.append((task_id, lease_token, result))
        return True
...
def test_worker_claims_evaluates_and_completes():
    tasks = FakeTasks(SimpleNamespace(id="t1", fen="fen", attempts=1, lease_token="l1"))
    worker = make_worker(tasks, lambda fen, config: 34)
    assert worker.process_once() is True
    assert tasks.recovered == 1
    assert tasks.completed == [("t1", "l1", 34)]
...
def test_worker_retries_engine_failure_then_fails_on_second_attempt():
    ...
    tasks.task = SimpleNamespace(id="t1", fen="fen", attempts=2, lease_token="l2")
    assert worker.process_once() is True
...
def test_success_persistence_error_does_not_become_engine_retry():
    ...
    tasks = BrokenTasks(SimpleNamespace(id="t1", fen="fen", attempts=1, lease_token="l1"))
    worker = make_worker(tasks, lambda fen, config: 34)
